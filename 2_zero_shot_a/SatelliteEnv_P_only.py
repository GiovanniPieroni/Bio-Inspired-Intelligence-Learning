...
class SatelliteEnv(gym.Env):
    def __init__(
        ...
        termination_tol: float = 1e3,
        penalty_weights: list = [1.0, 0.0],
        delta_p_min: float = -150000.0,
        delta_p_max: float = 150000.0,
        initial_keplerian: np.ndarray = None,
    ):
        ...
        self.initial_keplerian = (
            initial_keplerian
            if initial_keplerian is not None
    ...
    def reset(self, seed=None, options=None, target_delta_p=None):
        ...
        # Sample or assign zero-shot target delta_p
        if target_delta_p is not None:
            self.current_target_delta_p = float(target_delta_p)
        else:
            if not hasattr(self, "_target_sign_flip"):
                self._target_sign_flip = 1.0
            self._target_sign_flip *= -1.0  # Alternate sign on every reset
            min_abs_delta_p = 5000.0  # 5 km exclusion zone
            max_abs_delta_p = max(abs(self.delta_p_min), abs(self.delta_p_max))
            mag = float(self.np_random.uniform(min_abs_delta_p, max_abs_delta_p))
            self.current_target_delta_p = mag * self._target_sign_flip

        # Dynamically compute target orbit MEE
        target_keplerian = self.initial_keplerian.copy()
        target_keplerian[0] += self.current_target_delta_p
        self.target_orbit = conversion.keplerian_to_mee(target_keplerian)
        ...
        return self.normalized_rl_state.copy(), {
            "target_delta_p": self.current_target_delta_p
        }
    ...
    def step(self, action):
        ...
        mee_perturbation_penalty = -5.0 * np.sum(
            (self.raw_errors[2:5] / self.state_scales[2:5]) ** 2
        )
        ...
        e_reached = bool(
            abs(self.equinoctial_parameters[1] - self.target_orbit[1])
            < 0.008  # Small tolerance for eccentricity
        )
        terminated = bool(p_reached and e_reached)

        # ----------------------------------------------------------------------- # Terminal Reward Adjustments

        # Terminal Success Bonus (20.0)
        if terminated and not out_of_fuel:
            print("Target orbit reached!")
            reward += 20.0 - mee_perturbation_penalty

        # Terminal Failure Penalty (*10.0)
