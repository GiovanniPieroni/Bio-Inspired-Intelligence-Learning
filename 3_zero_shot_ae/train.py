...
target_keplerian = initial_keplerian.copy()
...
@torch.no_grad()
def evaluate_policy(actor, env, num_episodes=100, max_steps=250):
    rewards = []
    successes = []
    final_a_errors = []
    final_e_errors = []
    ...
    for _ in range(num_episodes):
        ...
        p_curr, f_curr, g_curr = env.equinoctial_parameters[0:3]
        e_curr = np.sqrt(f_curr**2 + g_curr**2)
        a_curr = (
            p_curr / (1.0 - (f_curr**2 + g_curr**2))
            if (f_curr**2 + g_curr**2) < 1.0
            else p_curr
        )

        a_target = env.target_keplerian[0]
        e_target = env.target_keplerian[1]

        final_a_errors.append(float(a_curr - a_target))
        final_e_errors.append(float(e_curr - e_target))

    rewards = np.array(rewards)
    abs_a_errors = np.abs(np.array(final_a_errors))
    abs_e_errors = np.abs(np.array(final_e_errors))
    ...
    print(
        f"Mean |a error|: {abs_a_errors.mean():.2f} m (max {abs_a_errors.max():.2f} m)"
    )
    print(f"Mean |e error|: {abs_e_errors.mean():.4e} (max {abs_e_errors.max():.4e})")
    print("------------------------------------------------------------\n")

    return {
        "success_rate": float(np.mean(successes)),
        "mean_reward": float(rewards.mean()),
        "mean_abs_a_error": float(abs_a_errors.mean()),
        "mean_abs_e_error": float(abs_e_errors.mean()),
    }
...
def train(seed: int, base_save_dir: str = None):
    if base_save_dir is None:
        base_save_dir = str(current_dir / "results_2")
    set_seed(seed)
    ...
    env = SatelliteEnv(
        ...
        termination_tol_a=cfg.rl.termination_tol_a,
        termination_tol_e=cfg.rl.termination_tol_e,
    ...
    # --- WARM-START WEIGHTS FROM 2_zero_shot_a ---
    pretrained_actor_path = (
        project_root / "2_zero_shot_a" / "results" / f"seed_{seed}" / "actor_best.pth"
    )
    pretrained_critic_path = (
        project_root / "2_zero_shot_a" / "results" / f"seed_{seed}" / "critic_best.pth"
    )

    if pretrained_actor_path.exists():
        print(
            f"[Seed: {seed}] Loading pretrained Actor weights from {pretrained_actor_path}..."
        )
        actor.load_state_dict(torch.load(pretrained_actor_path))

    if pretrained_critic_path.exists():
        print(
            f"[Seed: {seed}] Loading pretrained Critic weights from {pretrained_critic_path}..."
        )
        critic.load_state_dict(torch.load(pretrained_critic_path))
    ...
    print(
        f"[Seed: {seed}] Starting off-policy ADHDP max {MAX_EPISODES} episodes, {MAX_STEPS} steps per episode"
    )
    ...
    rewards_history = []
    final_a_error_history = []
    final_e_error_history = []
    actor_loss_history = []
    critic_loss_history = []
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not done and step < MAX_STEPS:
            ...
            current_a_err = abs(a_curr - a_target)
            current_e_err = abs(e_curr - e_target)
            ...
            if done:
                print(
                    ...
                    f"Target delta_a: {env.current_target_delta_a:.2f} m | "
                    f"Target delta_e: {env.current_target_delta_e:.4f} | "
                    f"final_a_error: {current_a_err:.2f} m | "
                    f"final_e_error: {current_e_err:.4e} \n "
                )
        ...
        if terminated:
            ...
            print(
                f"[Ep {episode}] Target reached! Reward: {reward:.4f} at step {step} | "
                f"a_error: {current_a_err:.2f} m | "
                f"e_error: {current_e_err:.4e}"
            )
        else:
            success_count = 0

        metric = float(
            (current_a_err / env.state_scales[0]) ** 2
            + (current_e_err / env.state_scales[1]) ** 2
        )  # Normalized joint error cost
        actor_scheduler.step(metric)
        rewards_history.append(episode_reward)
        final_a_error_history.append(current_a_err)
        final_e_error_history.append(current_e_err)
    ...
    np.save(
        os.path.join(save_dir, "final_a_error_history.npy"),
        np.array(final_a_error_history),
    )
    np.save(
        os.path.join(save_dir, "final_e_error_history.npy"),
        np.array(final_e_error_history),
    )
    np.save(
