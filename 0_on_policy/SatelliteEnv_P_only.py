...
class SatelliteEnv(gym.Env):
    ...
    def step(self, action):
        ...
        coasting_time = ((coast_action + 1.0) / 2.0) * (
            self.max_coast_fraction * current_T_orbital
        )
        ...
        error_reduction_reward = self.previous_state_cost - current_state_cost
        self.previous_state_cost = current_state_cost

        thrust_penalty = thrust_action**2
        reward = -(current_state_cost + 0.0 * thrust_penalty)

        # Terminazione basata sulla tolleranza vettoriale
        terminated = bool(
